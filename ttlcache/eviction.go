...
expiration time of its extries. When an expiry is reached the Timer will fire
...
//
// ExpiryHeap expects the caller to synchronize calls to most of its metehods. This
...
func (h *ExpiryHeap) Update(idx int, expiry time.Duration) {
	...
	// Ignore the new expiry if the time is earlier than the exist expiry
...
func (h *ExpiryHeap) notify() {
	// Send to channel without blocking. Skips sending if there is alreay
	// and item in the buffered channel.
	select {
